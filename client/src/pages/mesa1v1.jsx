...
function Mesa1v1({ roomId, gameState, onLeaveToRoomList }) {
  ...
  const j1Points = player1 ? state.pointsByPlayer?.[player1.id] ?? 0 : 0;
  const j2Points = player2 ? state.pointsByPlayer?.[player2.id] ?? 0 : 0;

  const trucoState = state.truco || { status: "idle", callerId: null, responderId: null };
  const isTrucoPending = trucoState.status === "pending";
  const isTrucoResponder = isTrucoPending && trucoState.responderId === socket.id;
  const isTrucoCallerWaiting = isTrucoPending && trucoState.callerId === socket.id;
  ...
  const isEnvidoResponder = isEnvidoPending && envidoState.responderId === socket.id;
  const isEnvidoCallerWaiting = isEnvidoPending && envidoState.callerId === socket.id;
  ...
  const isFlorResponder = isFlorPending && florState.florEnvidoResponderId === socket.id;
  const isFlorCallerWaiting = isFlorPending && florState.florEnvidoCallerId === socket.id;
  ...
  const bothPlayersSangFlor =
    state.players.length === 2 &&
    state.players.every((p) => !!florState.sungByPlayer?.[p.id]);
  ...
  const acceptedByMyTeam =
    !!trucoState.acceptedById && trucoState.acceptedById === socket.id;
  ...
  const viraPositionClass =
    roundStarterId === socket.id
      ? "left-4 bottom-4 sm:left-6 sm:bottom-6"
      : roundStarterId === opponent?.id
    ...
    return (
      <div className="relative w-[78px]" style={{ height: `${76 + (cards.length - 1) * stackStep}px` }}>
        {cards.map((card, index) => {
          const top = fromNorth
            ? (cards.length - 1 - index) * stackStep
            : index * stackStep;
          const zIndex = index + 1;
          return (
            <div
              key={`${card.playerId}-${card.value}-${card.suit}-${index}`}
              className={`absolute left-1/2 -translate-x-1/2 ${fromNorth ? "rotate-180" : ""}`}
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/55 px-4">
          <div className="w-full max-w-md rounded-2xl border border-emerald-200/30 bg-slate-900/95 p-5 text-white shadow-2xl">
            ...
            <p className="mt-1 text-center text-sm text-slate-300">
              {state.matchWinnerId
                ? `${state.players.find((p) => p.id === state.matchWinnerId)?.name || "Jugador"} llegÃ³ a 12 puntos.`
      ...
      <div className="fixed right-2 top-2 z-50 w-[132px] rounded-lg border border-emerald-200/35 bg-emerald-50/95 p-1 text-slate-800 shadow-[0_8px_18px_rgba(0,0,0,0.3)] sm:right-4 sm:top-4 sm:w-[195px] sm:p-1.5">
        ...
        <div className="space-y-0.5 sm:space-y-1">
          <div className="flex items-center justify-between rounded-md bg-white/80 px-1 py-0.5 sm:px-1.5 sm:py-1">
            ...
            <div className="text-xs font-extrabold leading-none sm:text-sm">{j1Points}</div>
          </div>
          <div className="flex items-center justify-between rounded-md bg-white/80 px-1 py-0.5 sm:px-1.5 sm:py-1">
            <div className="truncate text-[11px] font-semibold sm:text-xs">
              <span className="mr-1 inline-block rounded bg-emerald-700 px-1 py-0.5 text-[8px] text-white sm:text-[9px]">
            ...
            <div className="text-xs font-extrabold leading-none sm:text-sm">{j2Points}</div>
          </div>
        </div>
      </div>
